            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    if($_SERVER['SERVER_ADDR']=='42.121.84.129'){
                    	exec("/usr/bin/convert -resize '".$thumbWidth[$i]."x".$thumbHeight[$i].">' -quality 95 {$filename} { $thumbPath.$thumbname.'.'.$thumbExt }  ");
                    }else{
                    	Image::thumb ( $filename, $thumbPath.$thumbname.'.'.$thumbExt, "", $thumbWidth[$i], $thumbHeight[$i] );
                    }
                    */
