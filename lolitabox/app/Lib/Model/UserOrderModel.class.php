...
class UserOrderModel extends Model
{
    ...
    {
        $needGoToPayGateway = true;
        $user = D("DBLock")->getSingleUserLock($userid);
        $balance = $user["balance"];
        if($balance == 0){
            return true;
        }
        $toUseBalance = 0;
        if ($balance >= ($order['cost'] + $data["postage"])) {
            $data['pay_bank'] = null; //如果使用礼品卡余额全额支付，清除支付方式
            $data['giftcard'] = $order['cost'] + $data["postage"];
            $this->save($data);
            $toUseBalance = $order['cost'] + $data["postage"];
            $needGoToPayGateway = false;
        } else {
            $data['giftcard'] = $balance;
            $this->save($data);
            $toUseBalance = $balance;
        }
        $sql = "UPDATE `users` SET balance=balance-" . $toUseBalance . " WHERE userid=" . $userid;
        $this->db->execute($sql);
        return $needGoToPayGateway;
    }
