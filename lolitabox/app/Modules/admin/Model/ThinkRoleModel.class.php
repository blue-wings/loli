<?php
// 角色模型
class ThinkRoleModel extends CommonModel {
...
function setGroupActions($groupId,$actionIdList)
{
	...
	function setGroupUsers($groupId,$userIdList)
	{
		...
		$rs = $this->execute('INSERT INTO '.$this->tablePrefix.'role_user (role_id,user_id) SELECT a.id, b.id FROM '.$this->tablePrefix.'role a, '.$this->tablePrefix.'admin b WHERE '.$where);
		if($result===false) {
			return false;
		}else {
			return true;
