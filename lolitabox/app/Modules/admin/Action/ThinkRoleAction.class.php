<?php
// 角色模块
class ThinkRoleAction extends CommonAction {
	...
	public function action(){
		...
		if(!empty($moduleId)) {
			...
			$map['level']=3;
			$map['pid']=$moduleId;
			$actionList	=	$node->where($map)->field('id,title,type')->order("type")->select();
		}
	...
	//游览所有权限
	public function readrolelist()
	{
		$groupId =  $_REQUEST['id'];//roleid
		if (empty($groupId)) {
			$this->error('参数不正确！');
		}
		$appList = array();
		$group = D('ThinkRole');
		$appList	=	$group->getGroupAppList($groupId);

		foreach ($appList as $key=>$value){
			//读取当前项目的授权模块列表
			$appList[$key]['module'] =	$group->getGroupModuleList($groupId,$value['id']);
			foreach ($appList[$key]['module'] as $k=>$v){
				$appList[$key]['module'][$k]['action'] = $group->getGroupActionList($groupId,$v['id']);
			}
		}
