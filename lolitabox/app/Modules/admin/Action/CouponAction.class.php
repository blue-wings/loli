...
class CouponAction extends CommonAction{
	...
	public function giftcard(){
		$giftcard_mod=M("Giftcard");
		import("@.ORG.Page");
		$where=array();
		$count=$giftcard_mod->where($where)->count();
		$p = new Page ( $count, 15);
		$page = $p->show ();
		$list=$giftcard_mod->where($where)->limit($p->firstRow . ',' . $p->listRows)->order("add_datetime DESC")->select();
		$this->assign("list",$list);
		$this->assign ("page", $page );
		$this->display();
	}
	...
	public function createGiftcard(){
		...
		for($x=0;$x<$amount;$x++){
			...
			$data["card_pwd"]=rand(100000,999999);
			$data["price"]=$price;
			$data["indate"]=$indate;
			$data["remark"]=$remark;
			$data["add_datetime"]=$add_datetime;
			if($giftcard_mod->add($data)) {
				$cards[]=$data;
			}
			else {
				$x--;
		...
		$result=$amount."张（".$price."）礼品卡已经生成!";
		$this->success($result);
	}
	
	//生成并验证礼品卡卡号
	public function getCardid(){
		$cardid=rand(60000000,69999999);
		$giftcard_mod=M("Giftcard");
		$giftcard_mod->find($cardid);
		if($giftcard_mod->find($cardid)){
			$this->getCardid();
		}
		else {
			return $cardid;
